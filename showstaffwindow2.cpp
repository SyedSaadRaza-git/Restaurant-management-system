...
showstaffwindow2::showstaffwindow2(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::showstaffwindow2)
{
    ui->setupUi(this);
    this->showFullScreen();
...
void showstaffwindow2::on_Back_clicked()
{
    this->close();
...
void showstaffwindow2::on_searchButton_clicked()
{
...
void Sort(QVector<QMap<QString, QString>> &staffData, const QVector<QString> &positionOrder) {
...
void showstaffwindow2::on_showAllButton_clicked()
{
    QSqlDatabase db=QSqlDatabase::database("restaurant_connection");;
    if (!connectToDatabase()) {
    ...
    if(query.next()) {
        ...
        ui->totalStaffLabel->setText("Total Staffs: " + QString::number(total));
    }
